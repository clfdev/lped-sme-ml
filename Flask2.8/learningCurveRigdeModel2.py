...
# Adjust train sizes based on dataset size - use fewer points for clearer spacing
train_sizes = np.linspace(0.1, 1.0, 5)  # Reduced to 5 points for more spacing
...
def analyze_learning_curves(model, X, y, model_name, random_state=RANDOM_STATE):
    ...
    # Add more space between tick marks - explicitly set x-ticks
...
plt.title('R² vs Regularization Strength', fontsize=16)
...
plt.xlabel('Alpha (regularization strength)', fontsize=14)
plt.ylabel('R² Gap (Train - Test)', fontsize=14)
plt.title('Overfitting vs Regularization Strength', fontsize=16)
plt.grid(True)
