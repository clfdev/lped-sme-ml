...
plt.tight_layout()
plt.savefig('correlation_matrices_comparison2.png', dpi=300, bbox_inches='tight')
...
def evaluate_model(data_df, model_name):
    ...
    # Get feature importances directly from gradient boosting model
    feature_importances = pd.DataFrame({
        'Feature': X.columns,
        'Importance': model.feature_importances_
    }).sort_values(by='Importance', ascending=False)
    
    # Calculate permutation importance for a more reliable measure
...
# Print feature importances (built-in from gradient boosting)
print("\nFeature Importance (Inverse Distance Model):")
print(inverse_results['feature_importances'])
print("\nFeature Importance (Lennard-Jones Potential Model):")
print(lj_results['feature_importances'])

# Print permutation importance
print("\nPermutation Importance (Inverse Distance Model):")
print(inverse_results['permutation_importances'])
print("\nPermutation Importance (Lennard-Jones Potential Model):")
print(lj_results['permutation_importances'])
...
plt.subplot(2, 2, 1)
inverse_results['feature_importances'].plot(x='Feature', y='Importance', kind='bar', ax=plt.gca())
plt.title('Feature Importance\n(Inverse Distance Model)', fontsize=12)
plt.tight_layout()

plt.subplot(2, 2, 2)
lj_results['feature_importances'].plot(x='Feature', y='Importance', kind='bar', ax=plt.gca())
plt.title('Feature Importance\n(Lennard-Jones Potential Model)', fontsize=12)
plt.tight_layout()

# Permutation importance
plt.subplot(2, 2, 3)
inverse_results['permutation_importances'].plot(x='Feature', y='Importance', kind='bar', ax=plt.gca())
plt.title('Permutation Importance\n(Inverse Distance Model)', fontsize=12)
plt.tight_layout()

plt.subplot(2, 2, 4)
lj_results['permutation_importances'].plot(x='Feature', y='Importance', kind='bar', ax=plt.gca())
plt.title('Permutation Importance\n(Lennard-Jones Potential Model)', fontsize=12)
plt.tight_layout()

plt.savefig('feature_importance_comparison2.png', dpi=300, bbox_inches='tight')
...
metrics_comparison['Improvement (%)'] = [
    ((lj_results['metrics']['r2'] - inverse_results['metrics']['r2']) / abs(inverse_results['metrics']['r2'])) * 100,
...
plt.tight_layout()
plt.savefig('performance_metrics_comparison2.png', dpi=300, bbox_inches='tight')

# Add scatter plot of predicted vs actual values
plt.figure(figsize=(14, 6))

# Inverse Distance Model
plt.subplot(1, 2, 1)
plt.scatter(inverse_results['y_test'], inverse_results['y_pred'], alpha=0.6)
plt.plot([min(inverse_results['y_test']), max(inverse_results['y_test'])], 
         [min(inverse_results['y_test']), max(inverse_results['y_test'])], 
         'r--', linewidth=2)
plt.xlabel('Actual LPED')
plt.ylabel('Predicted LPED')
plt.title(f'Inverse Distance Model\nR² = {inverse_results["metrics"]["r2"]:.4f}')

# Lennard-Jones Model
plt.subplot(1, 2, 2)
plt.scatter(lj_results['y_test'], lj_results['y_pred'], alpha=0.6)
plt.plot([min(lj_results['y_test']), max(lj_results['y_test'])], 
         [min(lj_results['y_test']), max(lj_results['y_test'])], 
         'r--', linewidth=2)
plt.xlabel('Actual LPED')
plt.ylabel('Predicted LPED')
plt.title(f'Lennard-Jones Model\nR² = {lj_results["metrics"]["r2"]:.4f}')

plt.tight_layout()
plt.savefig('prediction_scatter_comparison2.png', dpi=300, bbox_inches='tight')

print("\nAnalysis completed. Visualization images saved as:")
print("- correlation_matrices_comparison2.png")
print("- feature_importance_comparison2.png")
print("- performance_metrics_comparison2.png")
print("- prediction_scatter_comparison2.png")