...
def plot_calibration_curve(y_true, y_pred, uncertainties):
    ...
    # Plot calibration curve
    plt.plot(confidence_levels, empirical_coverage, 'bo-', linewidth=2, markersize=5, label='Modelo calibrado')
    plt.plot([0, 1], [0, 1], 'r--', linewidth=2, label='Calibração perfeita')
    
    plt.xlabel('Nível de confiança esperado', fontsize=14)
    plt.ylabel('Cobertura empírica', fontsize=14)
    plt.title('Curva de calibração da incerteza', fontsize=16)
    plt.grid(True, alpha=0.3)
    plt.legend(fontsize=12)
    plt.tight_layout()
    plt.savefig('calibration_curve_port.png', dpi=300)
...
plt.errorbar(x, Y_pred_test[sort_idx], yerr=1.96*uncertainty_results['cal_std'][sort_idx], 
             fmt='o', ecolor='blue', alpha=0.7, capsize=4, label='95% de intervalo de confiança')
plt.scatter(x, Y_test.values[sort_idx], color='red', label='Valores reais')
plt.scatter(x, Y_pred_test[sort_idx], color='blue', label='Valores previstos')

plt.xlabel('Índice de Amostra de Teste (ordenado por valor real)', fontsize=14)
plt.ylabel('LPED (kcal mol⁻¹ Bohr⁻³)', fontsize=14)
plt.title(f'Previsões do LPED com incerteza calibrada\nR² = {test_metrics["r2"]:.4f}, Incerteza média = {uncertainty_results["avg_rel_uncertainty"]:.2f}%', 
          fontsize=16)
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('predictions_with_uncertainty_port.png', dpi=300)
