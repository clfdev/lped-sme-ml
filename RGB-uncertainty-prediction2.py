...
plt.tight_layout()
plt.savefig('calibrated_prediction_uncertainty.png', dpi=300)

# Create histogram of uncertainty distribution
plt.figure(figsize=(10, 6))
plt.hist(cal_pred_std, bins=10, alpha=0.7, edgecolor='black')
plt.axvline(avg_uncertainty, color='red', linestyle='--', 
            label=f'Mean Uncertainty: {avg_uncertainty:.4f}')
plt.xlabel('Calibrated Prediction Standard Deviation (kcal mol⁻¹ Bohr⁻³)', fontsize=14)
plt.ylabel('Frequency', fontsize=14)
plt.title(f'Distribution of Calibrated Prediction Uncertainty (calibration factor: {cal_factor:.2f})', fontsize=16)
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('calibrated_uncertainty_distribution.png', dpi=300)

# Create scatter plot of uncertainty vs prediction value
plt.figure(figsize=(10, 6))

# Calculate the interquartile range of uncertainties
q1 = np.percentile(cal_pred_std, 25)  # 25th percentile
q3 = np.percentile(cal_pred_std, 75)  # 75th percentile
...
plt.scatter(abs(mean_pred), cal_pred_std, alpha=0.7, zorder=10)

plt.xlabel('|Predicted LPED| (kcal mol⁻¹ Bohr⁻³)', fontsize=14)
plt.ylabel('Calibrated Prediction Uncertainty (kcal mol⁻¹ Bohr⁻³)', fontsize=14)
plt.title('Calibrated Uncertainty vs. Magnitude of Prediction', fontsize=16)
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('calibrated_uncertainty_vs_prediction.png', dpi=300)
