...
print(f"LPED = {intercept:.4f}", end='')
for index, row in coefficients.iterrows():
    if row['Coefficient'] >= 0:
        print(f" + {row['Coefficient']:.4f} × {row['Feature']}", end='')
    else:
        print(f" - {abs(row['Coefficient']):.4f} × {row['Feature']}", end='')
print()
...
train_data.to_excel('training_data.xlsx', index=True, index_label='Sample Index')

print("\nTest Data:")
test_data = pd.concat([X_test, Y_test], axis=1)
print(test_data)
# Export test data to Excel with index
test_data.to_excel('test_data.xlsx', index=True, index_label='Sample Index')

# Export both sets to different sheets in a single Excel file with index
with pd.ExcelWriter('split_datasets.xlsx') as writer:
